...
(async () => {
    ...
    console.log(chalk.hex(COLOURS.SUCCESS)(separator));
    console.log(chalk.hex(COLOURS.PRIMARY)(welcomeMessage));
    console.log(chalk.hex(COLOURS.SUCCESS)(separator));
    const closeShell = (async () => {
        console.log(chalk.hex(COLOURS.PRIMARY)(goodByeMessage));
        console.log(chalk.hex(COLOURS.PRIMARY)(figlet.textSync('DONE')));
        shell.exit(0);
    });
    const noInputError = (async () => {
        console.log(chalk.hex(COLOURS.WARNING)(noInputMessage));
        shell.exit(0);
    });
        ...
        for (let templateConfig of templates.variants) {
            ...
            for (let fname of templateConfig.files) {
                ...
                if (fname === "index.ejs") {
                    ...
                    filePath = `${templateConfig.path}/${fname}`;
